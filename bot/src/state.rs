...
/// Most recent `N` closed trades are kept in memory; older entries are
/// evicted. History persistence lives in a future PR (SQLite).
pub const TRADES_CAPACITY: usize = 5_000;
...
pub struct AppState {
    ...
    /// Live account snapshot (balance, equity, etc.) — populated on auth.
    /// `None` until the first `ProtoOATraderReq` response lands.
    pub account: RwLock<Option<dto::Account>>,

    /// Bounded history of closed trades, newest at the back. Capacity is
    /// `TRADES_CAPACITY`; older entries are evicted on push.
    pub trades: RwLock<VecDeque<dto::Trade>>,
...
impl AppState {
    pub fn new(
        ...
        Arc::new(Self {
            ...
            trades: RwLock::new(VecDeque::with_capacity(TRADES_CAPACITY)),
            ws_tx,
            cmd_tx,
        })
    }
